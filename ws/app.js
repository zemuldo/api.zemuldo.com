...
const {titles} = require('../db/db');
const {topics} = require('../config/env')
            ...
            switch (o.pups) {
                case 'chat':
                    processRequest(msg)
                        .then(function (answer) {
                            if (ws.readyState === 1) {
                                ws.send(JSON.stringify({type: 'bot', pups: 'bot', msg: answer}))
                            }
                        })
                        .catch(function (err) {
                            console.log(err)
                ...
                case 'exploreBlogs':
                    titles.find().limit(10).toArray()
                        .then(function (o) {
                            ws.send(JSON.stringify({type: 'exploreBlogs', pups: 'exploreBlogs', msg: o}))
                        })
                        .catch(function () {
                ...
                case 'topicDetails':
                    for(let i=0;i<topics.length;i++){
                        titles.find({topics:topics[i].key}).count()
                            .then(function (n) {
                                if(n>0){
                                    ws.send(JSON.stringify({type: 'topicDetails', pups: 'topicDetails', msg: [{key:topics[i].key,blogs:n}]}))
                                }
                            })
                            .catch(function (e) {
                                console.log(e)
